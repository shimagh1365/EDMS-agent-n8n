...
// 1. Start Command - Fetches Entity data and displays the first dropdown (THE ORIGINAL VERSION)
app.message('start', async ({ message, say }) => {
    try {
        const userId = message.user;
        
        // 1. Fetch Entity Options from Master Options Sheet (Assuming EntityName is the tab name)
        const entityRows = await getSheetData('EntityName!A1:A'); 
        const entityOptions = createSlackOptions(entityRows, 0, 0); // Column A is both value and text

        userDataStore[userId] = { state: 'ASKING_ENTITY', userId: userId };

        await say({
            text: "Please choose the Entity Name:",
            blocks: [{
                type: "section",
                text: { type: "mrkdwn", text: "*1. Choose the Entity Name:*" },
                accessory: {
                    type: "static_select",
                    action_id: "entity_name_selected", // NEW action_id to catch the click
        ...
        await say("⚠️ Error starting. Please ensure the 'EntityName' sheet exists and the API key is valid.");
    }
});
